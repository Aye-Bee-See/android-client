...
**Phase 6b: the group's side of end-to-end encryption (3 to 4 days).**
Open the group key from the member's key bundle; read letters through the group envelope, and a managed writer's letters through the writer key the group holds; send as the group, anonymously, or for a managed writer with the right envelopes; record replies encrypted to the writer; add a writer with a keypair made on the device; make claim tokens on the device (`tokenHash` plus the private key wrapped under the token) since an end-to-end server never sees them; group-key bootstrap and hand-out to members (`PUT /auth/chapter-keys`, `PUT /auth/member-key`); forwarding to a partner group (`POST /messaging/envelope`).
