...
6a. **Tokens outlive the database.** A token issued before a `DB_RESET` (or a restore from backup) keeps working if `JWT_SECRET` is unchanged, and maps to whichever account now holds that id. Ask: bump the sessions revocation marker whenever the database is created or restored. Until then, during development, sign out in the app after resetting the API's database.
...
6b. **Rule tags.** Agreed on 17 Sep 2026 that facility rules become tags or enums so clients can act on them (hide attachments where images are refused, warn on page limits, offer translation where a language is required). Not on the API's `main` yet. Asks: publish the tag vocabulary as soon as it is fixed, and have every client ignore tags it does not know, so a new tag never breaks an installed app.
